...
public class StripsPlanImpl extends StripsPlan {
	...
	protected String convertPlanBody() {
				...
				planStep = planStep.replace("_DETDUP_0", "");
				String[] splitPlan = planStep.split("\\s+");
				String newPlanStep = "";
				int i =0;
				for (String step: splitPlan){
					if (i == 0){
						newPlanStep += step.substring(1) + "(";
					} else {
						newPlanStep += step;
					}

					if (i> 0 && i < splitPlan.length - 1){
						newPlanStep += ", ";
					}
					i++;
				}
